...
def enhanced_declumping(binary_mask, vacuole_img, min_distance=20, h_minima=None):
    ...
    if vacuole_img is not None:
        ...
        for peak in intensity_peaks:
            is_far_enough = all(
                np.sqrt((peak[0] - ep[0]) ** 2 + (peak[1] - ep[1]) ** 2)
                >= min_distance / 2
                for ep in all_peaks
            )
            if is_far_enough:
...
def shape_based_declumping(
    binary_mask, vacuole_img=None, min_distance=20, proportion_threshold=0.12
):
...
def segment_vacuoles_improved(
    ...
    if use_multi_threshold:
        binary_mask = multi_threshold_segmentation(
            vacuole_img, sigma=threshold_smoothing_scale
        )
    else:
        vacuole_log = exposure.adjust_log(vacuole_img + 1)
        ...
        for cell_id in cell_ids:
            ...
            if cell_id in grouped.groups:
                ...
                if nuclei_detection:
                    summary_entry.update(
                        {
                            "total_nuclei_in_vacuoles": cell_vacuoles[
                                "nuclei_count"
                            ].sum(),
        ...
        cell_summary = [
            {
                ...
                "cell_area": None,
                "total_vacuole_area": sum(
                    v["vacuole_area"] for v in vacuole_cell_mapping
                )
                if vacuole_cell_mapping
                else 0,
                "vacuole_area_ratio": None,
                "mean_vacuole_diameter": vacuole_cell_df["vacuole_diameter"].mean()
            ...
            cell_summary[0].update(
                {
                    ...
                    "multinucleated_vacuole_count": len(
                        vacuole_cell_df[vacuole_cell_df["nuclei_count"] > 1]
                    ),
                }
            )
        elif nuclei_detection:
    ...
    if segment_cells:
        print(
            f"Kept {total_kept} out of {num_vacuoles} detected vacuoles ({total_kept / num_vacuoles * 100:.1f}%)"
        )
        print(
            f"Discarded {num_vacuoles - total_kept} vacuoles that didn't meet criteria"
        )
    else:
...
def create_empty_results(
    ...
    if segment_cells and cell_masks is not None:
        ...
        if cell_masks is not None:
            ...
            raise ValueError(
                "Need either cell_masks, cytoplasm_masks, or image_shape to determine image dimensions"
            )
