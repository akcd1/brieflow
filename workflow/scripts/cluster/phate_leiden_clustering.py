...
phate_leiden_clustering = phate_leiden_clustering.merge(
    uniprot_data, how="left", left_on=snakemake.params.perturbation_name_col, right_on="gene_name"
).drop(columns="gene_name")

# calculate potential to nontargeting
average_distance_df = calculate_potential_to_nontargeting(
    potential_df, snakemake.params.control_key, perturbation_name_col=snakemake.params.perturbation_name_col
)

# merge clustering data with potential_df
phate_leiden_clustering = phate_leiden_clustering.merge(
    average_distance_df, how="left", on=snakemake.params.perturbation_name_col
)
